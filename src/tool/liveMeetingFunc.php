...
function generateAttachment($type,$meeting,$last=false){
  $countGood=0;
	echo '<td width="33%" style="position:relative;min-width:300px;padding:5px;border-radius:10px;height:100%;background-color:'.((isNewGui())?'var(--color-light)':'#EEE').';vertical-align:top;">'
        ...
        .'<div style="width:85%;" dojoType="dijit.form.TextBox" id="new'.$type.'" name="new'.$type.'" >'
        .'  <script type="dojo/method" event="onKeyUp" >'
        .'    var inputValue = dijit.byId(\'new'.$type.'\').get("value");'
        .'    if(event.keyCode==13){'
        .'      if(inputValue != ""){'
        .'        addNewLinkMeeting(\'new'.$type.'\',\'refreshBottom'.$type.'\','.$meeting->idProject.');'
        .'      }'
        .'    }'
        .'  </script>'    
        .'</div>'
        .'<a id="addLink" onClick="addNewLinkMeeting(\'new'.$type.'\',\'refreshBottom'.$type.'\','.$meeting->idProject.');" title="'. i18n('addNewLinkMeeting') .'" >'
        .formatSmallButton('Add')
        .'</a></div>'
        .'';
	echo '<div style="width:100%;height:300px;overflow-x=hidden;overflow-y:auto;" class="liveMeetingBottomDivs">';
       ...
       .'</a>
        ...
        <td class="linkData"> 
          <table style="width:100%;">
            <tbody>
              <tr>
                <td style="cursor:pointer;" onclick="showDetail(\'refreshBottom'.$type.'\',1,\''.$type.'\',false,'.$obj->id.');">'.$obj->name .'</td>
                <td style="cursor:pointer;float:right;" class="iconGoto16 iconGoto iconSize16 imageColorNewGui" style="z-index:500;width:16px;height:16px;;" title="" onClick="gotoElement(\''.$type .'\','.htmlEncode($obj->id).');">&nbsp;</td>
              </tr>
            </tbody>
          </table>
        </td>
...
function generateBottom($meeting){
  ...
  echo '<div style="padding:0 10px;height:100%;"><table width="100%" style="height:100%;padding-bottom: 10px;max-height: 250px;"><tr>';
