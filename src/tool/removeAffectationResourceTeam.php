...
if($autoAffectationPool=="IMPLICIT"){
  ...
  foreach ($listAffPool as $valAffPool){
    if((pq_strtotime($valAffPool->startDate) >= pq_strtotime($obj->startDate)  and pq_strtotime($valAffPool->endDate) <= pq_strtotime($obj->endDate))
    OR (!$obj->startDate and $obj->endDate) OR (!$obj->startDate and pq_strtotime($obj->endDate) >= pq_strtotime($valAffPool->endDate))
    OR (!$obj->endDate and pq_strtotime($obj->startDate) <= pq_strtotime($valAffPool->startDate))){
      $valAffPool->delete();
    }
  }
}
