...
function formatLetterThumb($idUser,$size,$userName=null,$floatLetter="right", $idTicket=null, $rounded=false) {
  global $print;
	...
	if($print){
      ...
      if($rounded){
        $result.=' onMouseEnter="showToolTip(\'tooltipUserThumb_'.$idTicket.'\');" onMouseLeave="hideToolTip(\'tooltipUserThumb_'.$idTicket.'\', 200);"';
      }else{
	...
	$result.='</span>';
	if($rounded){
	  $result .='<div class="comboButtonInvisible" dojoType="dijit.form.DropDownButton" id="tooltipUserThumb_'.$idTicket.'" name="tooltipUserThumb_'.$idTicket.'" style="position:absolute;top:'.($size*0.75).'px;left:-'.($size*0.25).'px;height: 0px; overflow: hidden;" >';
	  $result .='  <div dojoType="dijit.TooltipDialog" id="dialogTooltipUserThumb_'.$idTicket.'" style="cursor:pointer;" onMouseEnter="clearTimeout(hideToolTipTimeout);" onMouseLeave="hideToolTip(\'tooltipUserThumb_'.$idTicket.'\', 200);" >';
...
function formatUserThumbRounded($userId,$userName,$title,$size=15,$radius=5, $float='left', $textSize=7, $maxWidth=50) {
  global $print;
  if ($print) return "";//$userName;
  $arrayColors=array('#1abc9c', '#2ecc71', '#3498db', '#9b59b6', '#34495e', '#16a085', '#27ae60', '#2980b9', '#8e44ad', '#2c3e50', '#f1c40f', '#e67e22', '#99CC00', '#e74c3c', '#95a5a6', '#d35400', '#c0392b', '#bdc3c7', '#7f8c8d');
...
function formatColorThumb($col,$val, $size=20, $float='right',$name="", $idTicket="", $rounded=false) {
  ...
  if($name!="" and $rounded){
    ...
    $res .='    <table style="width:100%"><tr><td style="padding-right: 5px;">'.formatIcon($class, $size).'</td><td style="min-width:100px;max-width:200px;">'.i18n('col'.ucfirst($col)).' '.$name.'</td></tr></table>';
    $res .='  </div>';
    $res .='</div>';
  }else{
    $res .='&nbsp';
...
function formatColorThumbRounded($col,$val,$float='right',$name="",$radius=5,$textSize=7) {
  $class=pq_substr($col,2);
  if (! SqlElement::class_exists($class)) return '';
  $color=SqlList::getFieldFromId($class, $val, 'color');
  if (! $color) return '';
  $res='<div style="color:' . htmlForeColorForBackgroundColor ( $color ) . ';background:'.$color.';';
  $res.='float:'.$float.';border-radius:'.$radius.'px;font-size:'.$textSize.'pt;padding:4px 8px 4px 8px;border-radius:'.$radius.'px"';
  $res.=' title="'.i18n('col'.ucfirst($col)).' '.$name.'">'.$name.'</div>';
  return $res;
}
...
function activityStreamDisplayNote ($note,$origin,$width=false,$noteDiscussionMode=false){
  ...
  if($origin=='activityStream'){
    $ticketName = '<span class="streamLink" style="margin-left:22px;position:relative;" onClick="gotoElement(\''.htmlEncode($objectClass).'\',\''.htmlEncode($note->refId).'\')">'
        .'<div style="position:absolute;top:0px;left:-24px;">'. formatIcon($note->refType, 16) . '</div>' . i18n($note->refType) . ' #' . $note->refId ;
    $ticketName.='</span>';
  }else{
    $ticketName = '<span class="streamLink" style="margin-left:22px;position:relative;" onClick="gotoElement(\''.htmlEncode($objectClass).'\',\''.htmlEncode($note->refId).'\')">'
