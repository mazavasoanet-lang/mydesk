 ...
 */
  ...
  $param['DbName']=pq_strtolower($param['DbName']);
  $param['DbPrefix']=pq_strtolower($param['DbPrefix']);
  ...
  foreach ($param as $id=>$val) {
    $ct=$ctrl[$id];
    if (pq_substr($ct,0,1)=="=") {
      if ( pq_strpos($ct, '=' . $val . '=')===false) {
        showError("incorrect value for '" . $label[$id] . "', valid values are : " . pq_str_replace("="," ",$ct));
        $error=true;
      }
  ...
  try {
   ...
   if (isset($param['SslKey']) and $param['SslKey'] and isset($param['SslCert']) and $param['SslCert'] and isset($param['SslCa']) and $param['SslCa']) {
    ...
    $connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  	...
  	if ($dbType=='pgsql' and pq_strpos($e->getMessage(), $param['DbUser'])>0 
  	    and (pq_strpos($e->getMessage(), "does not exist")>0 or pq_strpos($e->getMessage(), "n'existe pas")>0)) {
      ...
      showError(pq_utf8_encode($e->getMessage()).'<br/>dsn = '.$dsn);
  	  if ($dbType=='mysql') {
        exit;
      }
  	}
  ...
  if ( ! $baseExists ) {
  	try {
  	  $dbName=$param['DbName'];
  	  $dbName=pq_str_replace(chr(8),'',$dbName);
  	  $dbName=$connexion->quote($dbName);
  	  $dbName=pq_trim($dbName,"'");
      $query='CREATE DATABASE ' . $dbName;
      if ($dbType=='mysql') {
        $query.=' DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;';
  ...
  if ($param['logFile']) {
    ...
    if (! $error) {
      $logFile=pq_str_replace('${date}',date('Ymd'),$param['logFile']);
  ...
  writeFile('// =======================================================================================' . "\n", $paramFile);
  foreach ($param as $id=>$val) {
    if ($pname[$id]=='paramDbPrefix') $val=pq_strtolower($val);
