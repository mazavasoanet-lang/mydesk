...
//   	$queryWhere.=" OR (refType='Milestone' and ".getAccesRestrictionClause('Milestone',$table,$showIdleProjects).") )";
 ...
 // Apply restrictions on Filter
  ...
  $querySelectLeftAct=", $actTable.idStatus as activitystatus, $actTable.idActivityType as activitytype"
  ...
  if($outMode != 'csv' && getSessionValue('showAllGanttLines')!='true') $query.= "\n LIMIT ".$limitDisplayQuery;
      ...
      while ($line = Sql::fetchLine($result)) {
        ...
        $line['overallprogress']='';
      	...
      	if ( $largeQuery and (
    	    ...
    	    echo '}';
    	    $cp++;
    	    if($cp==$limitDisplay && getSessionValue('showAllGanttLines')!='true'){
...
//         }
        $refItem=$line['reftype'].'_'.$line['refid'];
        $refItemCollapsed=pq_str_replace(array('Fixed','Replan','Construction'), 'Project', $refItem);
        if (isset($collapsedList['Planning_'.$refItemCollapsed]) and !isset($directObjectParents['#'.$line['id']]) ) {
        	echo ',"collapsed":"1"';
