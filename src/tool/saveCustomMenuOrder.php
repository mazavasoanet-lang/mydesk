...
if(!$idRow)$idRow=0;
$idRowSource=RequestHandler::getValue('idRowSource');
if(!$idRowSource)$idRowSource=0;
$copy=RequestHandler::getBoolean('copy');
...
if ($copy) {
  ...
  $where="idUser=".getSessionUser()->id." and idRow =".$idRow." and name in ('".$customArray."')";
  $customMenu=new MenuCustom();
  $customMenuList=$customMenu->getSqlElementsFromCriteria(null, false, $where, 'sortOrder');
  foreach ($customMenuList as $menu) {
  ...
  $customArray = array_flip($customArrayOrder);
  $customArray = implode("','", $customArray);
  $where="idUser=".getSessionUser()->id." and idRow =".$idRowSource." and name in ('".$customArray."')";
  $customMenu=new MenuCustom();
  $customMenuList=$customMenu->getSqlElementsFromCriteria(null, false, $where, 'sortOrder');
  foreach ($customMenuList as $menu) {
  	$menu->sortOrder=$customArrayOrder[$menu->name];
