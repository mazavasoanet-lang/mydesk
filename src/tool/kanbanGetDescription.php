...
if ($kanbanFullWidthElement == "on") {
  echo pq_str_replace('<img ','<img style="max-width:'.($width-10).'px;" onClick="showImage(\'Note\',this.src,\' \');" ',$result);
} else {
  if(pq_strpos($result, '<img') == -1){
    $text = new Html2Text ($result);
    $descr = nl2br($text->getText());
  }else{
    $descr=pq_str_replace('<img ','<img style="max-width:'.($width-10).'px;" onClick="showImage(\'Note\',this.src,\' \');" ',$result);
  }
