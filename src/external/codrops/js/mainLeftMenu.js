...
;(function(window) {
  ...
  MLMenu.prototype._init = function() {
    ...
    this.menus.forEach(function(menuEl, pos) {
      ...
      links.forEach(function(linkEl, lPos) {
        ...
        if (submenu) {
          var classIcon=linkEl.firstChild.className;
          var iconNmae=classIcon.substring(0,(classIcon.indexOf('iconSize16')-1));
          var pushMe = {"menu":submenu, "name": linkEl.textContent, "id": idName, "icon":iconNmae};
          if (submenus[pos]) {
      ...
      submenus.forEach(function(subMenuEl, menu_root) {
        subMenuEl.forEach(function(subMenuItem, subPos) {
          ...
          if (subMenuItem.menu == menu_x) {
            ...
            self.menusArr[pos].icon = subMenuItem.icon;
          }
        });
      });
    });
   ...
   if(currentMenu.getAttribute('data-menu')!='main'){
     this.isAnimating = true;
      ...
      if( this.breadScrumLeft ) {
          ...
          iconClass= idx ? this.menusArr[idx].icon : 'iconHome';
          if(iconClass!=='iconReportSubMenu'){
            this.breadScrumLeft.removeChild(this.breadScrumLeft.lastElementChild);
            alreadyIconSubReport=false;
          }
      }
  ...
  var alreadyIconSubReport=false;
...
//    divBcl.className='icon'+iconClass+' iconSize22 iconBreadSrumb';
    var iconClass=  (idx)? ''+this.menusArr[idx].icon : 'iconHome';
    if(iconClass=='iconReportSubMenu'){
      if(alreadyIconSubReport){
        return ;
      }else{
        alreadyIconSubReport=true;
      }
    }
    var divBcl = document.createElement('div');

    divBcl.className=iconClass+' iconSize22 iconBreadSrumb';
    ...
    divBcl.addEventListener('click', function(ev) {
      ...
      if(idx==0)alreadyIconSubReport=false;
