...
class TCPDF {
	...
	public function getRawCharWidth($char) {
		if (is_float($char)) $char=intval($char);
	  if ($char == 173) {
			...
			try {
				...
				for ($xpx = 0; $xpx < $wpx; ++$xpx) {
					for ($ypx = 0; $ypx < $hpx; ++$ypx) {
						...
						if (is_float($alpha)) $alpha=intval($alpha);
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($dest[0] != 'F') {
			//$name = preg_replace('/[\s]+/', '_', $name);                // PBER - Changed to allow spaces in file name
			$name = preg_replace('/[^a-zA-Z0-9 _\.-]/', '', $name);       // PBER - Changed to allow spaces in file name
		}
		if ($this->sign) {
