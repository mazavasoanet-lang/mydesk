...
class pDraw
{
	...
	function drawPolygon($Points, array $Format = [])
	{
		...
		if (!$NoFill) {
			...
			if (count($Points) >= 6) {
			  if (version_compare(phpversion(), '8.1.0', '<')) ImageFilledPolygon($this->Picture, $Points, count($Points) / 2, $FillColor);
				else ImageFilledPolygon($this->Picture, $Points, $FillColor);
			}
		}
	...
	function drawRoundedRectangle($X1, $Y1, $X2, $Y2, $Radius, array $Format = [])
	{
		...
		if ($this->Antialias) {
			...
			imageline($this->Picture, intval($X1 + $Radius), intval($Y1), intval($X2 - $Radius), intval($Y1), $ColorA);
			imageline($this->Picture, intval($X2), intval($Y1 + $Radius), intval($X2), intval($Y2 - $Radius), $ColorA);
			imageline($this->Picture, intval($X2 - $Radius), intval($Y2), intval($X1 + $Radius), intval($Y2), $ColorA);
			imageline($this->Picture, intval($X1), intval($Y1 + $Radius), intval($X1), intval($Y2 - $Radius), $ColorA);
		}
	...
	{		
		...
		foreach($Positions as $Yp => $Bounds) {
			...
			imageline($this->Picture, intval($X1), intval($Yp), intval($X2), intval($Yp), $ManualColor);
		}
	...
	{		
		...
		if ($this->Antialias) {
			...
			imagerectangle($this->Picture, intval($X1), intval($Y1), intval($X2), intval($Y2), $this->allocateColor($R, $G, $B, $Alpha));
		}
	}
	...
	function drawLine($X1, $Y1, $X2, $Y2, array $Format = []) # FAST
	{
		...
		if ($this->Antialias == FALSE && $Ticks == NULL) {
			...
			$Color = $this->allocateColor($R, $G, $B, $Alpha);
			imageline($this->Picture, intval($X1), intval($Y1), intval($X2), intval($Y2), intval($Color));
			return 0;
		}
	...
	function drawFilledCircle($X, $Y, $Radius, array $Format = [])
	{
		...
		for ($i = 0; $i <= $Radius * 2; $i++) {
			...
			$this->Mask[intval($X - $XPos)][intval($YPos)] = TRUE;
			$this->Mask[intval($X + $XPos)][intval($YPos)] = TRUE;
			imageline($this->Picture, intval($X - $XPos), intval($YPos), intval($X + $XPos), intval($YPos), $Color);
		}
	...
	function drawText($X, $Y, $Text, array $Format = [])
	{
		...
		imagettftext($this->Picture, $FontSize, $Angle, intval($X), intval($Y), $C_TextColor, $FontName, $Text);
		$this->Shadow = $Shadow;
	...
	function drawAntialiasPixel($X, $Y, array $Format = [])
	{
		...
		if (!$this->Antialias) {
			...
			imagesetpixel($this->Picture, intval($X), intval($Y), $this->allocateColor($R, $G, $B, $Alpha));
			return 0;
		}
	...
	function allocateColor($R, $G, $B, $Alpha = 100)
	{
		if (!isset($this->aColorCache["$R.$G.$B.$Alpha"])){
			...
			$this->aColorCache["$R.$G.$B.$Alpha"] = imagecolorallocatealpha($this->Picture, intval($R), intval($G), intval($B), intval(1.27 * (100 - $Alpha)));
		}
	...
	function drawArrow($X1, $Y1, $X2, $Y2, array $Format = [])
	{
		...
		if (version_compare(phpversion(), '8.1.0', '<')) ImageFilledPolygon($this->Picture, $Points, 4, $this->allocateColor($FillR, $FillG, $FillB, $Alpha));
		else ImageFilledPolygon($this->Picture, $Points, $this->allocateColor($FillR, $FillG, $FillB, $Alpha));
	...
	function modulo($Value1, $Value2)
	{
		...
		return (floor($Value2) == 0) ? 0 : (intval($Value1) % $Value2);
	...
	function drawXThreshold($Value, array $Format = [])
	{
		...
		if ($Caption == NULL) {
		  $ValueDisp=round($Value,0);
			if (isset($Data["Abscissa"])) {
				$Caption = (isset($Data["Series"][$Data["Abscissa"]]["Data"][$ValueDisp])) ? $Data["Series"][$Data["Abscissa"]]["Data"][$ValueDisp] : $ValueDisp;
			} else {
				$Caption = $ValueDisp;
			}
		}
	...
	function drawStackedAreaChart(array $Format = [])
	{
		...
		foreach($SerieOrder as $Key => $SerieName) {
			...
			if ($Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"]) {
				...
				$YZero = $this->scaleComputeY([0], ["AxisID" => $Serie["Axis"]]); // PBER FIX for Scale strating at 1
				$this->DataSet->Data["Series"][$SerieName]["XOffset"] = 0;
				if ($Data["Orientation"] == SCALE_POS_LEFTRIGHT) {
					($YZero < $this->GraphAreaY1 + 1) AND $YZero = $this->GraphAreaY1 + 1;
					($YZero > $this->GraphAreaY2 - 1) AND $YZero = $this->GraphAreaY2 - 1;
