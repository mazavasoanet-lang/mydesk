...
// ============================================================================**********
  ...
  public function setDates($calendarDate) {
    $year=pq_substr($calendarDate,0,4);
    $month=pq_substr($calendarDate,5,2);
    $day=pq_substr($calendarDate,8,2);
  ...
  private function updateLeaveLeaveEarnedPlannedWork($result, $delete) {
    if (pq_strpos($result, "OK")===false and pq_strpos($result,"NO_CHANGE")===false) {return $result;}
    // Something changes in calendar
    if (pq_strpos($result,"NO_CHANGE")===false) {
...
//  	$crit=array('isOffDay'=>'1', 'idCalendarDefinition'=>'1');
        if (!pq_trim($idCalDef)) $idCalDef='1';
