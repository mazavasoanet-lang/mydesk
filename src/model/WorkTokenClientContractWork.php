...
class WorkTokenClientContractWork extends SqlElement {
  ...
  */
  ...
  public function save() {
    ...
    $idClientContract = SqlList::getFieldFromId('WorkTokenClientContract', $this->idWorkTokenClientContract, 'idClientContract');
    $idWorkTokenClientContractList = SqlList::getListWithCrit('WorkTokenClientContract', array('idClientContract'=>$idClientContract), 'id');
    $idWorkTokenClientContractList = (count($idWorkTokenClientContractList)>1)?implode(',', $idWorkTokenClientContractList):$this->idWorkTokenClientContract;
    $where = "idWorkTokenClientContract in ($idWorkTokenClientContractList)";
    $clientContract = new ClientContract($idClientContract, true);
    $clientContract->tokenUsed = $this->sumSqlElementsFromCriteria('workTokenMarkupQuantity', null, $where);
