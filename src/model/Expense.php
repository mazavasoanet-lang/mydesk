...
// ============================================================================**********
  ...
  public function save() {
    ...
    if (get_class($this) == 'ActivityExpense') {
      $pe=SqlElement::getSingleSqlElementFromCriteria('ActivityPlanningElement', array('refType'=>'Activity','refId'=>$this->idActivity));
      if (!$pe->id) debugTraceLog("Expense.php save() - cannot retreive ActivityPlanningElement with refType='Project' and refId=$this->idActivity");
      else $pe->updateExpense();
      if ($old->idActivity and $old->idActivity!=$this->idActivity) {
        $peOld=SqlElement::getSingleSqlElementFromCriteria('ActivityPlanningElement', array('refType'=>'Activity','refId'=>$old->idActivity));
        if (!$peOld->id) debugTraceLog("Expense.php save() - cannot retreive old ActivityPlanningElement with refType='Activity' and refId=$old->idActivity");
