...
// END - ADD BY TABARY - NOTIFICATION SYSTEM      
  ...
  public function save() {
    ...
    if (getLastOperationStatus($result)=='OK' and property_exists($this, 'tags')) {
      ...
      $resultTag = Tag::saveTagList($this->tags,$old->tags,get_class($this));
  ...
  public function simpleSave($withoutDependencies=false) {
  	if ( isset($debugTraceUpdates) and $debugTraceUpdates==true) {
  	  if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
  	    debugTraceLog("Start SIMPLE SAVE for ".get_class($this)." #".$this->id.((SqlElement::is_a($this, 'PlanningElement'))?" $this->refType #$this->refId":''));$startMicroTime=microtime(true);
  	}
    ...
    if (isset($debugTraceUpdates) and $debugTraceUpdates==true) {
      if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
        debugTraceLog("  End SIMPLE SAVE for ".get_class($this)." #".$this->id.((SqlElement::is_a($this, 'PlanningElement'))?" $this->refType #$this->refId":'')." => ".round((microtime(true) - $startMicroTime)*1000000)/1000000);
    }
  }
