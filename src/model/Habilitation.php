...
class Habilitation extends SqlElement {
  ...
  private static $_cachedHabilitation=array();
...
// ============================================================================**********
  ...
  public static function getAccesForProfileAndMenu($profile, $menu) {
    if (!isset(self::$_cachedHabilitation[$profile])) {
      $hab=new Habilitation();
      self::$_cachedHabilitation[$profile]=array();
      $list=$hab->getSqlElementsFromCriteria(array('idProfile'=>$profile));
      foreach ($list as $hab) {
        self::$_cachedHabilitation[$profile][$hab->idMenu]=$hab->allowAccess;
      }
    }
    if (isset(self::$_cachedHabilitation[$profile][$menu])) return self::$_cachedHabilitation[$profile][$menu];
    else return false;
  }
}
?>