...
class WorkTokenClientContract extends SqlElement {
  ...
  */
  ...
  public function save() {
    ...
    $clientContract->tokenOrdered = $this->sumSqlElementsFromCriteria('quantity', array('idClientContract'=>$this->idClientContract));
  ...
  public function delete(){
    ...
    $clientContract = new ClientContract($this->idClientContract);
    $clientContract->tokenOrdered = $this->sumSqlElementsFromCriteria('quantity', array('idClientContract'=>$this->idClientContract));
    $clientContract->tokenLeft = $clientContract->tokenOrdered - $clientContract->tokenUsed;
    $clientContract->save();
...
//           $valCal=$wTCCW->workTokenQuantity-$wTCCW->workTokenQuantity+($wTCCW->workTokenMarkupQuantity*$newWTM->coefficient);
      ...
      echo '<tr  style="width:100%;height:100%;text-align:right;">';
      echo  '<td class="assignData" style="white-space:nowrap; text-align:center; border-bottom:0px; border-right:0px; display: flex;">';
      if ($canUpdate) {
      ...
      echo  '<td class="assignData" style="background-color:'.$bgColorUsed.'; vertical-align: middle;text-align:center;">'.htmlDisplayNumericWithoutTrailingZeros($workTokenMarkupUsed).'</td>';
