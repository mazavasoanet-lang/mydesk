...
// ============================================================================**********
  ...
  public function saveWithPlanning() {
    ...
    if ($this->idProject == 0) {
      $crit=array('idProject'=>$this->idProject,'baselineDate'=>$this->baselineDate, 'idUser'=>$this->idUser);
    } else {
      $crit=array('idProject'=>$this->idProject,'baselineDate'=>$this->baselineDate);
    }
    if ($this->name=='automaticGlobalBaseline') {
      $crit['name']='automaticGlobalBaseline';
    }
    $list=$this->getSqlElementsFromCriteria($crit);
    foreach ($list as $base) {
      $base->deleteWithPlanning();
    }
   if (get_parent_class($this) == 'SqlElement') {
      $result = parent::save();
    } else {
      $this->save();
    }
  ...
  public function copyItem($itemFrom) {
    ...
    if ($this->idProject != 0 ) {
      ...
      if ($itemFrom=='PlannedWork') { // Also include existing real work
        $objFrom=new Work();
        $tableFrom=$objFrom->getDatabaseTableName();
        $colListWork=pq_str_replace(array('surbookedWork','surbooked'), array('0','0'), $colList);
