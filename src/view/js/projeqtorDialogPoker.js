...
function addPokerItem(classPokerItem,defaultPokerItem,idPokerSession) {
  var params="&objectClass=PokerSession&objectId=" + idPokerSession;
  loadDialog('dialogPokerItem',function() {
    ...
    if (classPokerItem) {
      ...
      message=i18n("dialogPokerItemRestricted",new Array(i18n(objectClass),objectId,i18n(classPokerItem)));
...
function savePokerItem() {
  if (dojo.byId("pokerItemRef2Id").value == "") return;
...
function editPokerItem(pokerItemId) {
  ...
  var callBack=function() {
    dojo.xhrGet({
      url:'../tool/getSingleData.php?dataType=editPokerItem&idPokerItem=' + pokerItemId + '&csrfToken=' + csrfToken,
      handleAs:"text",
      load:function(data) {
...
function pausePokerSession(idPokerSession) {
  dojo.xhrPost({
    url:'../tool/startPausePokerSession.php?idPokerSession=' + idPokerSession + '&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
      loadContent("objectDetail.php","detailDiv","listForm",null,null,null,null,null,true);
    }
...
function flipPokerVote(idPokerSession,idItem,itemList) {
  dojo.xhrPost({
    url:'../tool/flipPokerVote.php?idPokerItem=' + idItem + '&idPokerSession=' + idPokerSession + '&mode=flip&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
...
function resetPokerVote(idPokerSession,idItem,itemList) {
  dojo.xhrPost({
    url:'../tool/flipPokerVote.php?idPokerItem=' + idItem + '&idPokerSession=' + idPokerSession + '&mode=reset&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
      refreshPokerItemResult(idPokerSession,idItem,itemList);
    }
...
function openPokerItemVote(idItem,idPokerSession) {
  dojo.xhrPost({
    ...
    load:function(data) {
      dojo.xhrPost({
        url:'../tool/openPokerItemVote.php?idPokerItem=' + idItem + '&mode=open&csrfToken=' + csrfToken,
        handleAs:"text",
        load:function(data) {
          loadContent("objectDetail.php","detailDiv","listForm",null,null,null,null,null,true);
        }
...
function commitPokerItemVote(idPokerItem) {
  ...
  dojo.xhrPost({
    url:'../tool/openPokerItemVote.php?idPokerItem=' + idPokerItem + '&mode=close' + '&idComplexity=' + idComplexity + '&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
...
function pausePokerItemVote(idPokerItem) {
  dojo.xhrPost({
    url:'../tool/openPokerItemVote.php?idPokerItem=' + idPokerItem + '&mode=pause&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
      dijit.byId("dialogPokerCloseVote").hide();
      loadContent("objectDetail.php","detailDiv","listForm");
...
function refreshPokerVoteList(idPokerSession,idItem,itemList) {
  var url='../tool/refreshPokerVoteList.php?idPokerSession=' + idPokerSession + '&idItem=' + idItem + '&itemList=' + itemList;
...
function voteToPokerItem(idPokerSession,idItem,itemList,vote) {
  if (!vote) vote='';
  dojo.xhrPost({
    url:'../tool/voteToPokerItem.php?idPokerSession=' + idPokerSession + '&idItem=' + idItem + '&vote=' + vote + '&csrfToken=' + csrfToken,
    handleAs:"text",
    load:function(data) {
      pokerItemNav(idPokerSession,idItem,itemList,null);
    }
