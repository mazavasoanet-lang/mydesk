...
if(RequestHandler::isCodeSet('currentScreen')) {
  ...
  RequestHandler::setValue('currentScreen', $currentScreen);
  if ($currentScreen!='object') {
    Security::checkDisplayMenuForUser($currentScreen,true);
  }
}
...
if(RequestHandler::isCodeSet('paramActiveGlobal')){
  ...
  foreach (getSessionValue('userParamatersArray') as $code=>$val) {
    if (pq_substr($code,0,25)=='contentPaneRightDetailDiv') {
 ...
 <table width="100%">
    ...
    <tr height="<?php echo $iconSize+8; ?>px">  
      ...
      <?php if (Module::isModuleActive('moduleActivityStream')) {?>
        ...
        onclick="hideStreamMode('<?php echo ($activModeStream!='true')?'true':'false';?>','<?php echo $paramRightDiv?>','<?php echo $globalActivityStreamSize;?>',true, '<?php echo (RequestHandler::isCodeSet('currentScreen'))? $currentScreen : ""; ?>');">
          <table >
            <tr>
              <td >
    ...
    <td width="<?php echo (isIE())?37:35;?>px;" style="<?php echo (isNewGui())?'padding-left: 4px;':'';?>"> 
     ...
     <?php }?>
     <td width="<?php echo (isIE())?37:35;?>px">   
      <div id="fullTab" class="pseudoButton" onClick="fullTab();" title="<?php echo i18n('buttonFullTab')?>" style="height:28px; position:relative;top:-5px; z-index:30; width:30px; right:0px;"  >
          <table >
            <tr>
              <td style="width:28x;text-align:center">
                <div class="iconFullTab22 iconFullTab iconSize22 " style="position:absolute;top:5px;left:4px" ></div>
              </td>
            </tr>
          </table>    
	  </div>
